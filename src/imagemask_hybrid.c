...
int main(int argc, char *argv[])
{
    ...
    clock_gettime (CLOCK_MONOTONIC, &start);
        ...
        #pragma omp parallel for private(h, w, kx, ky, color_r, color_g, color_b) shared(bmp, array_local_me_pixels_R, array_local_me_pixels_G, array_local_me_pixels_B)
        for(h=start_row_height; h<end_row_height  ; h++){
            for(w=0; w<WIDTH; w++){
                color_r=0;
                color_g=0;
                color_b=0;
    ...
    //h for  sthn kx kai ky kanonika prepei na paei apo -1 ws 1 gia na ksekianw apo to mesaio stoixeio
        ...
        if(rank==0){
            ...
            if(metatropi == 0 ){
                SaveBMP(teliko_bmp,"output_hybrid.bmp");
                ...
                SaveBMP(gia_allagi,"output_hybrid.bmp");
                DestroyBMP(bmp);
                DestroyBMP(local_bmp);
                DestroyBMP(teliko_bmp);
...
#pragma omp parallel for private(h, w) shared(local_bmp,array_local_me_pixels_R,array_local_me_pixels_G,array_local_me_pixels_B)
    for(h=0;h<Height;h++){
        for(w=0;w<Width;w++){
//    [bmp->header.height_px][bmp->header.width_px]
//    for(h=0;h< bmp->header.height_px;h++){
//        for(w=0;w<bmp->header.width_px ;w++){
